...
**このスクリプトはもうサイトから参照していない**（ロゴは画像をやめて文字組みになった。
`src/components/SiteLogo.astro`）。下の ANCHORS は 2026-07-24 当時の参考パレットのままで、
2026-07-31 の付箋の 4 色には追従していない。出力の PNG を使い直すことがあれば、
`global.css` の `:root` に合わせて右側の色を入れ替えてから走らせること。
"""
