...
## 2 つの視点の呼び方（2026-07-31）

**GENERAL 側は日本語で「非エンジニア」と呼ぶ。「ふつう」「一般」「世間」は使わない**
（ユーザーの明示的な指示。エンジニアが普通ではない、という含みが出るため）。
日本語の対比語は**エンジニア / 非エンジニア**の 2 つだけ。

- 行き先は カードの読み上げ（`aria-live` / `aria-label`）・辞書エントリの節見出し・
  JSON-LD の `DefinedTermSet` 名と FAQ の質問文・`llms.txt` と `*.md` の節見出し・
  OGP 画像の右パネル・タグページの meta description。
  `grep -rn ふつう src/` が空であることで担保している
- **英語ラベルの `ENGINEER` / `GENERAL` は変えない**（付箋・透かし・バッジ・
  `CARDS` のキー）。変わったのは日本語の言い方だけ
- 例外は 2 つ。**語の定義文に出てくる専門用語**（`root.md` の `一般ユーザー`）と、
  **タグの `world` ラベル「日常側」**（飲料・食べ物という分野の区分けで、人を指さない）
...
| `src/lib/schema.ts` | JSON-LD。`DefinedTerm` を 2 ノード（エンジニア／非エンジニア）＋ `FAQPage` ＋ パンくず |
...
- **マーカーの色に意味がある。** `フロート`＝`#7FE3D4`（エンジニア視点の float）、
  `アイス`＝`#FFE66D`（非エンジニアの float）、`！`＝`#FF9EBB`（オチ・少し傾ける）。
