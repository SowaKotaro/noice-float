...
| `src/components/SiteLogo.astro` | タイトルロゴ。**画像ではなく文字組み**。寸法は全部 em で、親の font-size だけで拡縮する |
...
**タイトルロゴは画像をやめて文字で組んである**（ユーザー指示）。RetroUI のテーマと
同じ作り（純黒の枠線・ぼかしのないオフセット影・角丸 0）のチップを 2 段に並べ、
`その` / `フロート、`（青緑）/ `アイス`（黄・アイコン付き）/ `乗ってません！`（黒・`！`は赤）に
割ってある。次の 3 点はユーザー指示なので勝手に変えないこと。

- **横長で縦横比はおよそ 5 : 2**（実測 2.57 : 1）。比率は
  「CHIP の上下パディング」「小さい側の `text-[0.5em]` / `text-[0.46em]`」
  「チップの数」で釣り合わせてある。文言を足したら計測して取り直す
- **書体は Dela Gothic One**（重めのポップなゴシック / SIL OFL 1.1）。ロゴの 17 文字だけに
  削った 1.9KB の woff2 を自己ホストしている（`scripts/subset-logo-font.py`）
- **寸法は border・padding・影まですべて em**。置く側は font-size を決めるだけでよい
  （ヘッダー 16/19px、トップの見出し 32/46px）。px を混ぜるとヘッダーの小さいほうで破綻する
