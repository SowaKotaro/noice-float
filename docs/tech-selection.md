...
> **これは着手前の選定記録で、現状ではない。** 実装で変えた点は
> `progress.md` の「技術選定から変更した点」と「設計の変遷」にまとめてある。
> 特に **「5. データスキーマ」はもう当たらない** ── 部分列制約と `engineer.levels`
> （段階変形）は廃止して説明文 1 本の `engineer.description` になり、`category` も
> タグへ一本化、見出し語は英語からカタカナに変わっている。
