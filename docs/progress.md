...
- **重なる 2 枚**：前面カードだけが本文を出し、背面カードは見出しタブだけを上に
  のぞかせる。タブをクリックで前後入替。CSS Grid の同一セル重ね（`col-start-1 row-start-1`）
  ＋ `translate`/`rotate`/`z` で「重なった書類」を表現。高さの違いで破綻しないよう、
  **背面は見出しタブのみ描画**（本文は前面のときだけ出す）
...
- **状態はカードのローカル**（`useState`）。旧来のグローバル共有（nanostores + localStorage）は
  廃止。`client:load` で SSR＋ハイドレーションし、JS 無効でも初期状態は読める
- 一覧ページは操作なしの**静的プレビュー**（エンジニア前面・しっかり相当、ふつうタブがのぞく）。
  カード全体が詳細ページへのリンク
...
重なりは「背面カードの見出しタブが上にのぞく」形にした（高さ差で破綻しないための選択）。
本来ユーザーが選んだイメージは「書類が斜め下にはみ出す」だったので、実機で見て
のぞく向き・角度・オフセットを詰める余地がある。`MeaningCards.tsx` の
`translate`/`rotate` を触るだけで調整できる。
