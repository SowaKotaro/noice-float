...
- **重なる 2 枚**：どちらも本文を持つ完全なカード。CSS Grid の同一セル重ね
  ...
  **前面は右肩上がり（`rotate-[-3deg]`）、背面は右肩下がり（`rotate-[3deg]`）＋左上へ少し寄せる**。
  2 枚はグリッド stretch で**同サイズ（合同）**になるので、回転差で背面が四隅からはみ出す
  （左上のタイトル・左辺・右下）。紙が重なったような見え方。両カードの見出しが同時に見える。
  高さはエンジニア基準で揃うので、前後を入れ替えても土台は動かない。
  はみ出し分はコンテナの `px-8 pt-9 pb-6` で逃がしている
...
2 枚とも完全なカードで、前面=右肩上がり・背面=右肩下がり＋わずかに上にずらし、
両カードの左上タイトルが見えるようにした。のぞき具合・角度・入替アニメの速度は
`MeaningCards.tsx` の `POS_FRONT` / `POS_BACK` と `duration-200`、コンテナの `pt-7` を
触れば調整できる。実機で見て詰める余地あり。

高さの違うカードなので、背面が長い（エンジニアが背面）と下にも大きくのぞく非対称は
仕様（グリッドセルは常に高い方に揃うため土台は動かない）。
